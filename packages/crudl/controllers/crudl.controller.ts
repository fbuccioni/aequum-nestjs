...
import { string as stringUtil } from '@aequum/utils';
...
>(
    ...
    const ApiResponseNotFound = ApiResponse({
        status: 404,
        description: `${stringUtil.capitalize(options?.name.singular || '')} not found.`,
    });
    ...
    abstract class CRUDLController {
         ...
         * @protected
         */
        public readonly service: BaseCRUDLService;
        ...
        @Get(`:${idRouteParam}`)
        async retrieve(
            @Param(idRouteParam, options?.id.validationPipe as any) id: RetrieveDtoType['id'],
            @Req() request: any,
            ...args: any[]
        ): Promise<RetrieveDtoType> {
