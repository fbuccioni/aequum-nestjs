...
import { func as funcUtils } from "@aequum/utils";
...
export function postProcessClass(
    ...
    if (options.auth && options.auth.length) {
        const authDecoNames: Array<string> = (
            ...
            .map((dn) => `Api${swaggerUtils.authModName(dn)}Auth`);
...
export async function applyTransform<
    TransformFunc extends CRUDLTransformInputFunction | CRUDLTransformOutputFunction | undefined,
...
>(
    ...
    if(transform) {
        if (funcUtils.isAsync(transform))
